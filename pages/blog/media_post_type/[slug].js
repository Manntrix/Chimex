...
export default function BlogPage({
  ...
  return (
    <>
      <NextSeo
        title={`Media Type: ${unEntry(type?.name)}`}
...
export async function getStaticProps({ params }) {
  ...
  // const slug = 'news-press'

  /**
   * Tag
   */
  const typeRes = await fetch(
    `${process.env.NEXT_PUBLIC_WP_REST_URL}/wp/v2/media_post_type/?slug=${slug}`
  )
  ...
  const postsRes = await fetch(
    `${process.env.NEXT_PUBLIC_WP_REST_URL}/wp/v2/posts/?filter[orderby]=date&order=desc&media_post_type=${type.id}&per_page=100`
  )
...
export async function getStaticPaths() {
  const typesRes = await fetch(
    `${process.env.NEXT_PUBLIC_WP_REST_URL}/wp/v2/media_post_type`
  )
